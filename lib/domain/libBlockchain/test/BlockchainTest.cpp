...
namespace blockchainTest {
    ...
    bool BlockchainTest::runOnModule(Module &mod) {
        for(Function &fn : mod) {
            ...
            const BlkFunction *blkFn = blockchain->findFunction(*fnV);
            if(blkFn != nullptr) {
                //cout << blkFn->name() << " == " << fn.getName().str() << endl;
                fnCount[blkFn]++;
            }
            if(blockchain->isAnyExternalCall(*fnV)) {
                externalCallCnt++;
            ...
            for(auto &bb : fn) {
                for(auto &ins : bb) {
                    vanguard::Instruction* insV = llvmToVanguard.translateInstruction(&ins);
                    if(blockchain->writesStorage(*insV)) {
                        cout << fnName << endl;
                        ins.print(outs());
                        cout << endl;
                    ...
                    if(blockchain->readsStorage(*insV)) {
                        cout << fnName << endl;
                        ins.print(outs());
                        cout << endl;
